...
- (void)moveToNextItemIndex {
    if(self.isCounterclockwise){
        if(_topItemIndex <= 0){
            self.topItemIndex = (int)_items.count - 1;
        }else{
            self.topItemIndex--;
        }
    }else{
...
- (void)scrollTimerDidFire:(MSWeakTimer *)timer {
    dispatch_async(dispatch_get_main_queue(), ^() {
        ...
        if(self.isCounterclockwise){
            [_items[_topItemIndex] setFrame:CGRectMake(0.0f, -itemHeight, itemWidth, itemHeight)];
        }else{
            ...
            if(self.isCounterclockwise){
                for(int i = (int)_items.count - 1; i >= 0; i--){
                    ...
                    if(i == _items.count - 1){
                        ...
                        [_items[index] setFrame:CGRectMake(0.0f, itemHeight * i, itemWidth, itemHeight)];
                    }
                }
            }else{
                for (int i = 0; i < _items.count; i++) {
